...
- Something rewrites `c6remote-kicad/c6remote.kicad_pro` as a side effect of working on the design, and the rewrite always drifts the same way: `min_copper_edge_clearance` 0.5 → 0.025, `pth_inside_courtyard` / `npth_inside_courtyard` ignore → error, unused `highpower` / `lowpower` netclasses re-added, `Default` netclass priority reset. It came back twice in one session. Run `git diff c6remote-kicad/c6remote.kicad_pro` before every commit and `git checkout --` it unless the rule change is deliberate; committing the 0.025 value silently disarms the edge-clearance check. The culprit is NOT known, and two earlier guesses were checked and cleared on 2026-07-30, so do not repeat them. It is not the MCP server's own file handling: neither the canonical checkout at `~/dev/KiCAD-MCP-Server` nor the newer untracked copy in this repo contains any code that writes `min_copper_edge_clearance`, `pth_inside_courtyard`, `npth_inside_courtyard`, `highpower`, `lowpower`, or netclass `priority`, verified by grep across both. The only MCP path that round-trips this file at all is `create_netclass` → `persist_netclass_to_project`, which touches `net_settings.classes` only and re-dumps with `sort_keys=True`, so it can explain netclass churn and key reordering but not a design rule value. It is also not plain `kicad-cli`: `sch erc`, `pcb drc`, `sch export svg` and `sch export netlist` each leave the file byte identical, tested against a clean tree. That leaves the MCP tools that load and save through the `pcbnew` Python bindings, and the KiCad GUI writing the project file on save, as the untested candidates. Anyone who narrows it further should record the result here rather than re-deriving it. Four values in that file are now deliberate and must NOT be reverted, all set on 2026-07-30: `silk_over_copper` and `silk_edge_clearance` at `ignore` (Landon's call, see the DRC baseline above), `npth_inside_courtyard` and `track_not_centered_on_via` at `warning`, both stricter than before and both currently reporting zero. The `defaults.pads` block also drifts, `drill` 0.8 / `height` 1.27 / `width` 2.54 to 0.5 / 1.0 / 1.0, and that one is not from MCP or `kicad-cli`: those are the `TestPoint_THTPad_1.0x1.0mm_Drill0.5mm` dimensions, picked up because KiCad makes the last pad whose properties you opened the default for newly placed pads. KiCad's stock project template has no `pads` key at all, so the 0.8 / 1.27 / 2.54 values were only ever the compiled-in fallback serialized once. It affects nothing but the next hand-placed pad, so leave whichever value is there rather than churning the file.
