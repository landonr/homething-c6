...
- **Status lighting:** `D2` through `D5` are addressable `WS2812B-2020` LEDs in a 4-long cascade. `U1` `GPIO17` drives `D2` DIN on `led_1` with no series resistor, then `led_2`, `led_3`, and `led_4` carry DOUT to the next DIN. `D5` DOUT is a no-connect. All four VDD sit on `+3.3V`, one 100nF (`C2`) decouples the group.
