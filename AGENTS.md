...
- `c6remote-kicad/c6remote.kicad_pro` drift is root caused as of 2026-07-31 and now enforced, so do not re-investigate it: a save in eeschema rewrites the whole project file from the copy it loaded when the project was opened, silently reverting board settings changed in pcbnew since (proven by sentinel injection, one schematic save reverted `min_copper_edge_clearance`, both courtyard severities, an added netclass and the `Default` priority in a single write), no KiCad setting disables it, the historical `0.025` was an earlier uncommitted value of this file rather than any default, and `scripts/hooks/pre-commit` now runs `scripts/check-kicad-pro-drift.py` to block a commit whose staged project file moved on the guarded fields, so a genuinely deliberate rule change needs `--no-verify` and a note in the message. Four values in that file are now deliberate and must NOT be reverted, all set on 2026-07-30: `silk_over_copper` and `silk_edge_clearance` at `ignore` (Landon's call, see the DRC baseline above), `npth_inside_courtyard` and `track_not_centered_on_via` at `warning`, both stricter than before and both currently reporting zero. The `defaults.pads` block also drifts, `drill` 0.8 / `height` 1.27 / `width` 2.54 to 0.5 / 1.0 / 1.0, and that one is not from MCP or `kicad-cli`: those are the `TestPoint_THTPad_1.0x1.0mm_Drill0.5mm` dimensions, picked up because KiCad makes the last pad whose properties you opened the default for newly placed pads. KiCad's stock project template has no `pads` key at all, so the 0.8 / 1.27 / 2.54 values were only ever the compiled-in fallback serialized once. It affects nothing but the next hand-placed pad, so leave whichever value is there rather than churning the file.
