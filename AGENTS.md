...
- Autorouting: KiCadRoutingTools (Rust A* router, no Java) at `~/dev/KiCadRoutingTools`, NOT the MCP Freerouting autoroute (its jar needs Java 25, only 17 and 23 installed here). Run `~/dev/KiCadRoutingTools/.venv/bin/python ~/dev/KiCadRoutingTools/route.py <board.kicad_pcb>`, which writes `<board>_routed.kicad_pcb` (non-destructive, `--overwrite` replaces). Scope with `--nets "<glob>"`. Diff pairs: `route_diff.py`. Planes: `route_planes.py`. Deps live in that venv (numpy, scipy, shapely, prebuilt `grid_router.so` from `build_router.py`). Sync the board from the schematic (`sync_schematic_to_board`) BEFORE routing when the schematic changed, else the router routes stale topology.
