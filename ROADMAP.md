...
- [ ] **Shrink and simplify the D2 status LED block (schematic done, board pending).** Was 3 parts: SK6812MINI 3535 (12.25mm²) + R2 330Ω series on `led_1` + C2 1uF 0805. Now 2 parts. D2 → `WS2812B-2020` (Worldsemi, LCSC C965555, ~$0.05/reel part, 2.0x2.0 PLCC4, 4.0mm², same 1-wire 800kbit protocol, same 12mA/channel die class). Pin numbering is identical between the two KiCad symbols (1 DOUT, 2 VSS, 3 DIN, 4 VDD) and matches the WS2812B-2020 datasheet pin table, so no net remapping. Also replaces the old sourcing line, which was `Manufacturer Adafruit / MPN 4960`, an Adafruit 10-pack SKU rather than a reel an assembler can buy. R2 deleted: 330Ω is an LED current-limit value, not the 22-100Ω damping value a data line wants, and at ~5mm of trace on one rail no series R is needed; GPIO17 now drives DIN directly. C2 1uF 0805 → 100nF 0603 (`CC0603KRX7R9BB104`); the datasheet says external caps are not required, kept as local decoupling because the rail is under-spec (see High). Schematic verified 2026-07-25: ERC 0, netlist delta is exactly R2 gone and `Net-(D2-DIN)` merged into `led_1` (43 nets → 41), component delta exactly D2 value/footprint, C2 value/footprint, R2 removed. ESPHome `chipset:` moved `SK6812` → `WS2812`. Board work left: swap the D2 footprint, swap C2 to 0603, delete the R2 pads and its two stubs, reroute `led_1` to DIN, refill zones, re-run DRC.
- [ ] **Extend status light to a 4-LED chain (schematic done, board pending).** Added D3, D4, D5 (`WS2812B-2020`, same footprint and sourcing as D2) cascaded off D2: `GPIO17` → `led_1` → D2 DIN, D2 DOUT → `led_2` → D3 DIN, D3 DOUT → `led_3` → D4 DIN, D4 DOUT → `led_4` → D5 DIN, D5 DOUT no-connect. Still one GPIO, no new nets outside the three chain nets. All four VDD on `+3.3V`, all four VSS on GND, one 100nF (C2) for the group because the datasheet states external caps are not required. Schematic verified 2026-07-25: ERC 0, netlist 41 nets → 44, added nets exactly `led_2`/`led_3`/`led_4`, no existing net touched. ESPHome `num_leds: 1` → `4`. Budget: 144mA peak at full white on a 700mA XIAO 3V3 rail, and 2.4mA quiescent with all four dark, which is now the dominant idle load. Board work left: place D3-D5, route the three chain hops, refill zones, DRC. First mitigation if the chain misbehaves at 3.3V (see the under-spec VDD item under High) is a 100nF at the far LEDs, since each stage reshapes the signal from its own supply and a 4-long chain has three more marginal handoffs than one LED.
