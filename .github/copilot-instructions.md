...
- Git write operations from Codex can fail with `.git/index.lock` errors because sandbox blocks writes inside `.git`. If lock file is absent, treat this as permissions issue and rerun `git add`, `git commit`, or similar with escalation instead of assuming repo corruption.
