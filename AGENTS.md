...
Baseline as of 2026-07-30: `sch erc` 0 violations, `pcb drc` 0 errors and 0 unconnected items with 15 warnings (14 `lib_footprint_mismatch`, 1 `silk_overlap`). Treat those 15 as pre-existing unless the task is specifically about them, and a count above 15 is a regression introduced by the current task, not inherited.
...
- **`c6remote.kicad_pro` drift: root caused 2026-07-31. eeschema is the writer, and it clobbers the whole file from a stale in-memory copy.** A save in the schematic editor rewrites the entire project file from the copy eeschema read when it opened the project, discarding anything written to it since, including board settings eeschema has no business touching. Reproduced directly: four sentinel values were written into the file externally, then one ordinary eeschema save reverted all four in a single write, `min_copper_edge_clearance` 0.123 → 0.5, `pth_inside_courtyard` and `npth_inside_courtyard` error → ignore / warning, an added `sentinelclass` netclass deleted, and `Default` netclass priority 42 → 2147483647. Every value came back as whatever eeschema had loaded at open time.

  This explains the drift set that used to be described here as mysterious, `min_copper_edge_clearance` 0.5 → 0.025, courtyard severities to `error`, `highpower` / `lowpower` netclasses re-added, `Default` priority reset, and it explains "it came back twice in one session": those were not defaults and not a tool's hardcoded values, they were this project's own `.kicad_pro` contents at the moment that eeschema session opened it, resurrected on every subsequent save. `0.025` in particular is not KiCad's default at any version, the default was 0.01 when the setting was introduced and is 0.5 today, so it can only have been a real earlier working-tree value here. It was never committed, which is why it appears in no commit and in none of the 24 KiCad auto-backups in `c6remote-backups/`.

  Practical consequences. A long-lived eeschema window is a loaded gun pointed at Board Setup: change a design rule in pcbnew, save the board, then save the schematic hours later, and the rule silently reverts. If you change board settings deliberately, close and reopen eeschema, or save the schematic first and the board second. Keep running `git diff c6remote-kicad/c6remote.kicad_pro` before every commit, since that is what catches it; committing a stale `min_copper_edge_clearance` silently disarms the edge-clearance check. The upstream failure class is reported, see KiCad issues 12826, 23545 and 24314.

  Cleared by test on 2026-07-30 and 2026-07-31, so do not re-investigate these: the MCP server's own file handling (neither `~/dev/KiCAD-MCP-Server` nor the newer untracked copy in this repo contains any code writing these keys, grep verified, and the only path that round-trips the file at all is `create_netclass` → `persist_netclass_to_project`, which did nothing when driven); plain `kicad-cli` (`sch erc`, `pcb drc`, `sch export svg`, `sch export netlist` each leave it byte identical); the `pcbnew` Python API (`LoadBoard` plus `Save` or `SaveBoard`, with and without a zone refill, left it byte identical and even preserved an injected out-of-range sentinel); MCP `open_project`, `run_erc` and `save_project`; and a pcbnew GUI board save, which wrote `c6remote.kicad_pcb` while leaving all four sentinels in the project file intact. Four values in that file are now deliberate and must NOT be reverted, all set on 2026-07-30: `silk_over_copper` and `silk_edge_clearance` at `ignore` (Landon's call, see the DRC baseline above), `npth_inside_courtyard` and `track_not_centered_on_via` at `warning`, both stricter than before and both currently reporting zero. The `defaults.pads` block also drifts, `drill` 0.8 / `height` 1.27 / `width` 2.54 to 0.5 / 1.0 / 1.0, and that one is not from MCP or `kicad-cli`: those are the `TestPoint_THTPad_1.0x1.0mm_Drill0.5mm` dimensions, picked up because KiCad makes the last pad whose properties you opened the default for newly placed pads. KiCad's stock project template has no `pads` key at all, so the 0.8 / 1.27 / 2.54 values were only ever the compiled-in fallback serialized once. It affects nothing but the next hand-placed pad, so leave whichever value is there rather than churning the file.
